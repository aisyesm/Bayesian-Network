...
FE.add_values([['e', 'c', .2], ['e', '-c', .4], ['-e', 'c', 0.8], ['-e', '-c', .6]])
FF.add_values([['f',0.1], ['-f', 0.9]])
FG.add_values([['g',1], ['-g', 0]])
FH.add_values([['h',0.5], ['-h', 0.5]])
FI.add_values([['i', 'b', .3], ['i', '-b', .9], ['-i', 'b', 0.7], ['-i', '-b', .1]])

Q3 = BN('SampleQ4', [A,B,C,D,E,F,G,H,I], [FA,FB,FC,FD,FE,FF,FG,FH,FI])
...
if __name__ == '__main__':
    ...
    print('P(i|b) = {} P(-i|b) = {}'.format(probs[0], probs[1]))
    ...
    print('P(i|bh) = {} P(-i|bh) = {}'.format(probs[0],probs[1]))
    ...
    '''if probs1[0] == 0.5 and probs1[1] == 0.5 and probs2[0] == 0.0 and probs2[1] == 1.0:
      ...
      print("failed.")  '''
    print('P(h|a-b) = {} P(-h|a-b) = {}'.format(probs1[0],probs1[1]))
    ...
    '''if probs1[0] == 0.5 and probs1[1] == 0.5 and probs2[0] == 0.0 and probs2[1] == 1.0:
      ...
      print("failed.")  '''
    print('P(h|-b) = {} P(-h|-b) = {}'.format(probs1[0],probs1[1]))
'''
    print("Test 5 ....", end = '')
    S.set_evidence('-s')
    W.set_evidence('w')
